<script>
  ...
  function getMetadataUrl(metadata) {
    if (!metadata) return '';
  ...
  <div class="flex w-full flex-wrap justify-end gap-2 px-0 pt-1 pb-3">
    ...
    <div class="px-3">
      ...
      {#if record.ags}
        ...
        <!-- <a
          href="https://maps.mecklenburgcountync.gov/opendata-viewer?filter={record.title}"
          class="btn"
          target="_blank"
          on:click={() => sendEvent(record.title, "AGS View")}
        >
      ...
      {#if record.metadata}
        <a
          href="https://maps.mecklenburgcountync.gov/metadata/item/{getMetadataUrl(record.metadata)}"
          class="btn"
          target="_blank"
          title="Metadata"
        >
